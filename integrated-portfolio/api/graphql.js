  ...
  try {
    ...
    req.on('end', function() {
      try {
        ...
        if (query && query.indexOf('mutation') !== -1 && query.indexOf('sendContactMessage') !== -1) {
          ...
          var request = https.request(options, function(response) {
            ...
            response.on('end', function() {
              try {
                ...
                res.status(200);
                res.end(JSON.stringify({ data: { sendContactMessage: json } }));
              } catch (parseErr) {
                res.status(500);
                res.end(JSON.stringify({ errors: [{ message: 'Failed to parse contact API response' }] }));
              }
            });
          });
          ...
          request.on('error', function(error) {
            res.status(500);
            res.end(JSON.stringify({ errors: [{ message: 'Failed to call contact API', details: error.message }] }));
          });

          request.write(contactData);
          request.end();
          return;
        }
        ...
        // Handle profile and experiences (simple mock)
        if (query && query.indexOf('profile') !== -1) {
          res.status(200);
          res.end(JSON.stringify({ data: { profile: {
            id: 'p1',
            name: 'John Che Larracuente',
            title: 'Full Stack Developer',
            bio: 'I am a Full Stack Developer and Creative Technologist with expertise in JavaScript, React, Vue.js, and responsive design. Founder of Creations X (media agency) and Ori Company (consulting business), bringing 10+ years of experience in digital media and technology.'
          } } }));
          return;
        }

        if (query && query.indexOf('experiences') !== -1) {
          res.status(200);
          res.end(JSON.stringify({ data: { experiences: [] } }));
          return;
        }
    ...
    res.status(500);
    res.end(JSON.stringify({ error: 'Internal server error', message: 'Failed to process request' }));
  }
};